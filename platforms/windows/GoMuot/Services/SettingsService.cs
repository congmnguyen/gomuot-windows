...
{
    private const string RegistryKeyPath = @"SOFTWARE\GoMuot";
    private const string AutoStartKeyPath = @"SOFTWARE\Microsoft\Windows\CurrentVersion\Run";
    private const string AppName = "GoMuot";
    ...
    public void Save()
    {
        try
        {
            ...
            if (key != null)
            {
                key.SetValue(KeyInputMethod, (int)InputMethod.Telex, RegistryValueKind.DWord);
