...
fn ddddd_behavior() {
    ...
    // Debug step by step
    use gomuot_core::engine::Action;

    let mut screen = String::new();
    let inputs = ['d', 'd', 'd', 'd', 'd'];

    for c in inputs {
        let key = gomuot_core::utils::char_to_key(c);
...
fn bug11_arrow_shortcut() {
    use gomuot_core::engine::shortcut::Shortcut;
...
fn bug_nesue_to_neu_circumflex() {
    use gomuot_core::engine::Action;
    use gomuot_core::utils::telex_auto_restore;
...
fn issue197_mark_removal_after_backspace() {
    ...
    let key = gomuot_core::utils::char_to_key('<'); // '<' = DELETE
    let r = e.on_key(key, false, false);
    if r.action == Action::Send as u8 {
        for _ in 0..r.backspace {
            screen.pop();
    ...
    let key = gomuot_core::utils::char_to_key('r');
    let r = e.on_key(key, false, false);
    if r.action == Action::Send as u8 {
        for _ in 0..r.backspace {
            screen.pop();
...
fn issue197_caos_double_s_restore() {
    ...
    let key = gomuot_core::utils::char_to_key('s');
    let r = e.on_key(key, false, false);
    if r.action == Action::Send as u8 {
        for _ in 0..r.backspace {
            screen.pop();
...
// https://github.com/congmnguyen/gomuot-windows/issues/230
//
